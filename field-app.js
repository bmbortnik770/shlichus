...
/* ==========================================
   מצב מבצע נקי (Waze-style)
   ========================================== */
#f-mission-hud {
    animation: fadeIn 0.3s ease;
}

/* כפתור "הגעתי" — pulse עדין */
#f-mission-hud button[onclick*="nextMissionTarget"] {
    position: relative;
    overflow: hidden;
}
#f-mission-hud button[onclick*="nextMissionTarget"]::after {
    content: '';
    position: absolute;
    inset: 0;
    background: rgba(255,255,255,0.15);
    animation: missionBtnPulse 2s infinite;
...
/* ==========================================
   תיקונים: FAB, קהילה, טפסים
   ========================================== */

/* FAB — ימין תחתון מעל הניווט */
.super-fab-container {
    position: fixed !important;
    bottom: calc(var(--bottom-nav-h) + var(--safe-b) + 16px) !important;
...
/* הסר את הכחול מתחת לכרטיסי קהילה */
.community-list-item-wrapper {
    margin-bottom: 10px !important;
}
.swipe-bg { display: none !important; }
.community-card-front {
...
/* טופס משפחה — לא יוצא מהמסך */
.f-sheet {
    max-height: 88vh !important;
    overflow-y: auto;
}

/* action-sheet grid — ימין: משפחה+משימה, שמאל: מבצעים+מסלולים */
.action-grid {
...
.speed-dial {
    position: fixed;
    bottom: calc(var(--bottom-nav-h) + var(--safe-b) + 80px);
    right: 20px;
    z-index: var(--z-fab);
    display: flex;
    flex-direction: column;
...
.speed-dial-item:nth-child(1) { transition-delay: 0.05s; }
.speed-dial-item:nth-child(2) { transition-delay: 0.1s; }
.speed-dial-item:nth-child(3) { transition-delay: 0.15s; }
...
/* FAB מסתובב כשהתפריט פתוח */
.super-fab.dial-open { transform: rotate(45deg) !important; }

/* ==========================================
   Mission Planner לשוניות
   ========================================== */
.mp-tab {
    padding: 10px 16px;
    font-size: 14px;
...
.mp-content { animation: fadeIn 0.2s ease; }
...
.mp-select-item.selected {
    border-color: var(--accent);
    background: rgba(37,99,235,0.06);
}
.mp-select-item .mp-check {
...
.mp-select-item.selected .mp-check {
    background: var(--accent);
    border-color: var(--accent);
    color: white;
}

/* כפתור dark mode — מינימלי ליד sync */
.theme-anchor {
    position: fixed !important;
    top: calc(15px + var(--safe-t)) !important;
    right: 52px !important; /* ליד הsync-widget */
    left: auto !important;
...
/* תיקון מתכנן מבצעים — לא קופץ למעלה מדי */
#mission-planner-sheet.f-sheet {
    max-height: 75vh !important;
}

/* sync-widget מאחורי שורת חיפוש */
.sync-widget {
    z-index: calc(var(--z-top-bar) - 1) !important;
}

/* שורת חיפוש משימות מעל הסינכרון */
#view-tasks .f-top-bar {
    z-index: var(--z-top-bar) !important;
    position: relative;
}
...
.global-search-bar {
    ...
    left: 90px;   /* מרווח מהסינכרון + כפתור לילה */
    right: 90px;  /* מרווח מהצד השני */
    z-index: var(--z-top-bar);
    pointer-events: auto;
}
...
.global-search-input-wrap:focus-within {
    box-shadow: 0 0 0 2px var(--accent), 0 4px 16px rgba(37,99,235,0.15);
    border-color: var(--accent);
}
...
.search-result-item:active { background: var(--bg-body); }
...
/* במצב מבצע — הסתר חיפוש */
body.mission-active .global-search-bar { display: none; }

/* ==========================================
   Super FAB — הגדרה סופית
   ========================================== */
.super-fab-container {
    position: fixed !important;
    bottom: calc(var(--bottom-nav-h) + var(--safe-b) + 14px) !important;
...
.super-fab:active { transform: scale(0.9); }
.super-fab.dial-open { transform: rotate(45deg); }
.super-fab.state-crm { background: linear-gradient(135deg, var(--accent), #1d4ed8); }
.super-fab.state-route { background: linear-gradient(135deg, var(--success), #059669); }
