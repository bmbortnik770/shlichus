...
**קיים ב-v2 ‏(/shlichus/v2/), מכוסה בבדיקות אוטומטיות, בעיצוב המשוחזר מהממשק הקיים (7.7):**
מעטפת זהה לישן (ניווט גלולות עליון, סיידבר ימני עם ווידג'טים+התראות+FAB, צבעי המקור) · מפה (סמנים צבועים לפי סגנון, לוויין, לחיצה→מודל בניין) · מודל בניין (דירות+פרטים+ניווט Waze+הוספת משפחה) · אינדקס קהילה (חיפוש, פילטרי סגנון/תגיות, תצוגות חכמות מהישן, קשר-אחרון, bulk תגית/מחיקה-רכה, פעולות שורה חיוג/וואטסאפ, ייצוא CSV, משפחה חדשה) · כרטיס משפחה בטאבים (פרטים+עריכה, פעילות+תיעוד מהיר, משימות+הוספה, תרומות+הוספה, חיוג/וואטסאפ, ESC עם אישור) · משימות · אירועים · קנבן · מעגלים · תקשורת WhatsApp+מייל · הגדרות (תגיות/סגנונות/תבניות, גיבוי/שחזור JSON) · סנכרון דו-כיווני בטוח.

**נוסף בהמשך 7.7 (עד 33a7ff3):** עורך טריטוריה (ציור/שמירה/הסרה, אותו פורמט polygon) · מצבי צבע סגנון/קטגוריה/טריטוריה · סיווג קטגוריות במודל בניין · מנוע תאריך עברי (Intl) + טאב אבני דרך + תיעוד פטירה/יארצייט · Ctrl+K · מצב כהה · ייבוא CSV · פיצול כרטיס · מספר דירות (units) · שדות מוסד · ניהול סוגי אירועים · דיאגרמת עוגה בסיידבר · PWA (התקנה בטלפון).

**הפערים האחרונים (משניים):** סריקת POI בעורך הטריטוריה · ייבוא Google Sheets/Contacts · SMS/Twilio · בונה קהלים וסגמנטים · מסך הגדרות ניקוד (הציון עצמו מוצג) · מיתוג · אשף onboarding. **אף מסך ישן לא נגנז — הכל זמין במערכת הקיימת במקביל.**
